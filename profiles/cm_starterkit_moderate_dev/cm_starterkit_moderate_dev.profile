...
/**
 * @file
 * Enables modules and site configuration for a cm_starterkit_moderate_dev site installation.
 */

/**
 * Implementation of hook_menu().
 */
function cm_starterkit_moderate_dev_menu() {
  $items = array();

  $items['admin/documentation'] = array(
    'title' => 'Documentation',
    'description' => 'Link to Documentation on Drupal.org',
    'page callback' => 'cm_starterkit_moderate_dev_documentation',
 ...
 */
function cm_starterkit_moderate_dev_documentation() {
 ...
 * Allows the profile to alter the site configuration form.
 */
function cm_starterkit_moderate_dev_form_install_configure_form_alter(&$form, $form_state) {
 ...
 */
function cm_starterkit_moderate_dev_install_tasks_alter(&$tasks, $install_state) {

  // substitute our own finished step
  $tasks['install_finished'] = array(
    'display_name' => st('Finished Installation'),
    'display' => 1,
    'function' => 'cm_starterkit_moderate_dev_install_finished',
  );
    
} 

function cm_starterkit_moderate_dev_install_finished(&$install_state) {
 ...
 */
function cm_starterkit_moderate_dev_update_projects_alter(&$projects) {
  // Enable update status for the profile.
  $modules = system_rebuild_module_data();
  // The module object is shared in the request, so we need to clone it here.
  $profile = clone $modules['cm_starterkit_moderate_dev'];
  $profile->info['hidden'] = FALSE;
  _update_process_info_list($projects, array('cm_starterkit_moderate_dev' => $profile), 'module', TRUE);
}
 ...
 */
function cm_starterkit_moderate_dev_update_status_alter(&$projects) {
  $bad_statuses = array(
    UPDATE_NOT_SECURE,
    UPDATE_REVOKED,
  ...
  $make_filepath = drupal_get_path('module', 'cm_starterkit_moderate_dev') . '/drupal-org.make';
 ...
 */
function cm_starterkit_moderate_dev_block_info() {
 ...
 */
function cm_starterkit_moderate_dev_block_view($delta = '') {
  switch($delta){
